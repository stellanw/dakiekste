...
export default function Hero({ headline = "", text, textMobile, showLogoCarousel = true, maxWidthDesktop = "1200px", maxWidthMobile = "600px" }) {
  const [windowWidth, setWindowWidth] = useState(0);
  ...
  const resolvedText = isMobileOrTablet ? (textMobile ?? text) : text;

  return (
    <StyledHeadBoxContainer>
      <StyledTopTextWrapper $maxWidthDesktop={maxWidthDesktop} $maxWidthMobile={maxWidthMobile}>
        {headline && <h2>{headline}</h2>}
        {resolvedText && <h4>{resolvedText}</h4>}
      </StyledTopTextWrapper>
